...
// V1.0 - Merged with STM32_Code_V3.ino functionality
// 
// V1.0 Changes:
// 1. Improved PID controller from V3 (with Ki=0 for servo drives)
// 2. Non-blocking homing from V3
// 3. PID command handling from V3
// 4. Parallel axis motion (all 4 axes move independently)
// 5. EEPROM removed (use Flash or RAM only)
// 6. Compatible with Plugin.cs protocol
//
...
// modified version from 2025-01-15
...
// ============================================================================
// PID CONTROLLER - From V3 (Ki MUST be 0 for servo drives)
// ============================================================================
...
void timerISR_0() { processTimerISR(&axes[0]); }
void timerISR_1() { processTimerISR(&axes[1]); }
void timerISR_2() { processTimerISR(&axes[2]); }
void timerISR_3() { processTimerISR(&axes[3]); }
    ...
    switch (axis->homeSubState) {
        case HOME_SEEKING_LIMIT:
            if (axis->limitSwitchState == HIGH) {
                stopStepping(axis);
                axis->homeSubState = HOME_RETRACTING;
                axis->homeTargetPos = axis->currentPos - 
                    (HOME_DIRECTION == HIGH ? SAFE_DIST_IN_STEPS : -SAFE_DIST_IN_STEPS);
                axis->targetPos = axis->homeTargetPos;
...
// ============================================================================
// READY MOTION - With PID support from V3
// ============================================================================
    ...
    if (absError > 2) {
        ...
        if (axis->pidControlEnabled) {
            ...
            if (absError > STEPS_CONTROL_DIST) {
                ...
                desiredFreqHz = delayToFreq(axis->slowPulseDelay) + 
                    (delayToFreq(axis->fastPulseDelay) - delayToFreq(axis->slowPulseDelay)) * 
                    (absError - STEPS_CONTROL_DIST / 4) * 4 / STEPS_CONTROL_DIST;
            } else {
...
// ============================================================================
// COMMAND PROCESSING - With PID commands from V3
// ============================================================================
void ProcessCommand() {
    ...
    switch (pccmd.cmd) {
        ...
        case COMMAND::CMD_STORE_PID:
            // EEPROM not available on F407 in STM32Duino
            // Acknowledge command but don't store
            break;
